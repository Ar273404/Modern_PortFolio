...
const express = require("express");
const Analytics = require("../models/Analytical");
const { authenticateToken, requireAdmin } = require("../middleware/auth");
...
// Update analytics manually (Admin only)
router.post("/update", authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { visitorCount, projectClicks, contactRequests, blogViews } =
      req.body;
    ...
    if (visitorCount !== undefined)
      analytics.visitorCount = parseInt(visitorCount);
    if (projectClicks !== undefined)
      analytics.projectClicks = parseInt(projectClicks);
    if (contactRequests !== undefined)
      analytics.contactRequests = parseInt(contactRequests);
    if (blogViews !== undefined) analytics.blogViews = parseInt(blogViews);
...
router.post("/track", async (req, res) => {
  try {
    ...
    await Analytics.updateAnalytics(page, req.get("User-Agent"), req.ip);
